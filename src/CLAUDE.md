...
- **Brand cascade layer.** Atmospheres with `brand: '<id>'` set `<html data-brand="<id>">` via the same `applyTheme()` write site that owns `data-atmosphere` / `data-physics` / `data-mode`. The SCSS cascade is `global → physics → brand → atmosphere`. Brand profiles live in `src/config/brands/<brand>.ts` and carry sparse identity overrides (radii / motion / type-treatment / per-role weights) — never color, never spacing, never fonts. See [.claude/rules/theme-creation.md](../.claude/rules/theme-creation.md) "Brand Profile Overlay" for the discipline.
- **Retro physics floor wins over brand.** `_engine.scss`'s `emit-physics-retro-floor` re-asserts `--radius-*: 0` and `steps()` motion in a final block above any brand override. A brand profile setting `radii.full: '9999px'` will not round retro pills — that contract is non-negotiable.
