# Internationalization (i18n) — Future Package
...
**Status:** Parked research (captured 2026-04-15, reviewed 2026-04-19). Not a scheduled phase. Ships after all four roadmap phases land — i18n is additive, not foundational, and the monorepo must exist first to give the premium package a home. Revisit and formalize once the rest of the system is shipped.
**Consumes:** Requires the public/premium monorepo split (Phase 3) before the premium `@void-energy/i18n` package has somewhere to live. Some substrate prep (logical properties, `--direction-sign`, injectable KT segmenter) is cheap enough to pull forward opportunistically as those files are touched during earlier phases.
**Blocks:** Nothing hard — i18n is additive. CoNexus (Phase 4) benefits strongly from having this in place, and the brand website (`dgrslabs.ink`) can be the first consumer to prove the stack.
...
## Timing — what to prep opportunistically during earlier phases

**Ship the full i18n package after all four roadmap phases land.** i18n is additive, not foundational — it shouldn't share oxygen with the monorepo split, the mobile template, or the CoNexus migration. Each of those is already a large architectural change; stacking i18n on top doubles the risk surface.

**But bake i18n-readiness into earlier phases opportunistically**, without shipping the actual package. These are cheap refactors that touch files the earlier phases already touch:

| Do opportunistically during earlier phases | Defer until the i18n package ships |
|---|---|
...
Rationale: files touched by the monorepo split, mobile template, and CoNexus migration will be rewritten anyway — making them RTL-clean at the same time is nearly free. Retrofitting later means touching every package a second time.
...
This is the honest part — some of these are work-now, not work-later. Most slot into earlier phases naturally as those files are already being rewritten.
...
1. **Confirm substrate prep is complete.** Logical properties audited, `--direction-sign` token present, KT segmenter injectable. If not, close those gaps first.
...
These are the real trade-offs that need concrete decisions before implementation begins — not today.
...
- [../phase-3a-monorepo-structure.md](../phase-3a-monorepo-structure.md) — bake logical-properties audit + `--direction-sign` + segmenter interface in here
- [../phase-3b-premium-packages.md](../phase-3b-premium-packages.md) — `@void-energy/i18n` will become the 5th premium package
- [../phase-4-conexus-migration.md](../phase-4-conexus-migration.md) — CoNexus will be the first full-app consumer of the i18n stack
