...
- **Tailwind v4 bridge is token-driven.** Void Energy runs on Tailwind CSS v4 via `@tailwindcss/vite`. There is no `tailwind.config.mjs`. All theme registration and `@utility` / `@layer void-overrides` declarations live in `src/styles/tailwind-theme.css`, which reads CSS variables emitted by SCSS from `design-tokens.ts`. Add new values to `design-tokens.ts`; only touch `tailwind-theme.css` when changing the framework bridge itself (new namespace, utility override, layer-cascade fix).
- **Radius tokens in SCSS:** Default to `var(--radius-base)` for border-radius — it adapts per physics (8px glass, 8px flat, 0 retro). Use `var(--radius-full)` for pills. All radius tokens are force-zeroed in retro physics.
