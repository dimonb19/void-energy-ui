...
- Ambient backdrop layers (weather, mood, environment color grading, one-shot beats) → [packages/ambient-layers/AI-REFERENCE.md](packages/ambient-layers/AI-REFERENCE.md). Owns the variant/intensity vocabulary and lifetime semantics.
- Kinetic text (reveal styles, continuous effects, one-shot punctuation, TTS sync, inline style spans) → [packages/kinetic-text/AI-REFERENCE.md](packages/kinetic-text/AI-REFERENCE.md). Owns `revealStyle`, `speedPreset`, `activeEffect`, `punctuation`, and `styleSpans`.
...
- Optional kinetic emphasis on a hero phrase: `KineticText` with `revealStyle="rise"` or `"scale"` for the headline only. Reserve for one focal phrase per page — see [packages/kinetic-text/AI-REFERENCE.md](packages/kinetic-text/AI-REFERENCE.md).
- Optional `use:fontShift` on a single hero heading for variable-font weight modulation tied to scroll. Use scarcity — the effect dies under repetition.

Avoid:

- Building a fake design-system `Hero` component
- Recreating carousels or accordions without proving a real need
- Putting kinetic effects on every heading; the system reads as visual noise when overused
...
- Optional ambient color spill for hero / scene cards: `use:aura` (atmosphere-driven by default; pair with `extractAura` only when the color must come from the image). Apply to one focal surface per region — never on every card.
- Optional ambient backdrop for story-reading subroutes: `ambient.push('environment', 'night', 'medium')` plus an `atmosphere` variant when weather is in play. See [packages/ambient-layers/AI-REFERENCE.md](packages/ambient-layers/AI-REFERENCE.md).
- Narrative beats inside an article body: `KineticText` from `@void-energy/kinetic-text` for reveal-style emphasis on a phrase, sentence, or pull quote. See [packages/kinetic-text/AI-REFERENCE.md](packages/kinetic-text/AI-REFERENCE.md).

Avoid:

- Rebuilding cards for story browsing when `tile` already exists
- Hiding key state only in color; use labels and icons
- Stacking `use:aura` on every card in a rail — the rainbow-disco failure mode
...
- Sanitize the HTML **separately** before rendering (`sanitize-html`, DOMPurify, or a trusted-types policy). `.prose-untrusted` is the visual quarantine layer, not a security layer — it does not strip `<script>`, event handlers, or dangerous URL schemes.
