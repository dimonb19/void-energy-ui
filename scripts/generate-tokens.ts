...
//
// TO UPDATE: Modify 'design-tokens.ts' and run 'npm run build:tokens'
// ==========================================================================
...
`;
  ...
  Object.entries(tokens.themes).forEach(([themeName, config]) => {
    ...
    // Auto-inject default font mappings
...
async function main() {
  try {
    ...
    console.log(
      `   └─ 🎨 Styles Materialized: src/styles/config/_generated-themes.scss`,
    );

    // 3. Write JSON
    const jsonContent = generateRegistry(VOID_TOKENS);
    fs.writeFileSync(PATHS.json, jsonContent);
    console.log(`   └─ ⚙️  Logic Materialized:  src/config/void-registry.json`);
